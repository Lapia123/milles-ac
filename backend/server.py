...
@api_router.post("/loans/{loan_id}/swap")
async def swap_loan(loan_id: str, swap_req: LoanSwapRequest, user: dict = Depends(require_admin)):
    """Transfer/swap a loan to another borrower"""
    loan = await db.loans.find_one({"loan_id": loan_id}, {"_id": 0})
    if not loan:
        raise HTTPException(status_code=404, detail="Loan not found")
    
    if loan["status"] == LoanStatus.FULLY_PAID:
        raise HTTPException(status_code=400, detail="Cannot swap a fully paid loan")
    
    now = datetime.now(timezone.utc)
    
    # Get new vendor if provided
    new_vendor = None
    if swap_req.target_vendor_id:
        new_vendor = await db.vendors.find_one({"vendor_id": swap_req.target_vendor_id}, {"_id": 0})
    
    # Store old borrower info
    old_borrower = loan["borrower_name"]
    old_vendor_id = loan.get("vendor_id")
    
    # Update loan with new borrower
    update_doc = {
        "borrower_name": swap_req.target_borrower_name,
        "vendor_id": swap_req.target_vendor_id,
        "vendor_name": new_vendor["name"] if new_vendor else None,
        "updated_at": now.isoformat(),
        "last_swap_date": now.isoformat(),
        "swap_history": loan.get("swap_history", []) + [{
            "from_borrower": old_borrower,
            "from_vendor_id": old_vendor_id,
            "to_borrower": swap_req.target_borrower_name,
            "to_vendor_id": swap_req.target_vendor_id,
            "reason": swap_req.reason,
            "swapped_at": now.isoformat(),
            "swapped_by": user["name"]
        }]
    }
    
    # Adjust terms if requested
    if swap_req.adjust_terms:
        if swap_req.new_interest_rate is not None:
            update_doc["interest_rate"] = swap_req.new_interest_rate
        if swap_req.new_due_date:
            update_doc["due_date"] = swap_req.new_due_date
    
    await db.loans.update_one({"loan_id": loan_id}, {"$set": update_doc})
    
    # Record swap transactions
    outstanding = loan["amount"] + loan.get("total_interest", 0) - loan.get("total_repaid", 0)
    
    # Swap out from old borrower
    await db.loan_transactions.insert_one({
        "transaction_id": f"ltx_{uuid.uuid4().hex[:12]}",
        "loan_id": loan_id,
        "transaction_type": LoanTransactionType.SWAP_OUT,
        "amount": outstanding,
        "currency": loan.get("currency", "USD"),
        "from_borrower": old_borrower,
        "to_borrower": swap_req.target_borrower_name,
        "reason": swap_req.reason,
        "description": f"Loan transferred from {old_borrower} to {swap_req.target_borrower_name}",
        "created_at": now.isoformat(),
        "created_by": user["user_id"],
        "created_by_name": user["name"]
    })
    
    # Swap in to new borrower
    await db.loan_transactions.insert_one({
        "transaction_id": f"ltx_{uuid.uuid4().hex[:12]}",
        "loan_id": loan_id,
        "transaction_type": LoanTransactionType.SWAP_IN,
        "amount": outstanding,
        "currency": loan.get("currency", "USD"),
        "from_borrower": old_borrower,
        "to_borrower": swap_req.target_borrower_name,
        "reason": swap_req.reason,
        "description": f"Loan received from {old_borrower}",
        "created_at": now.isoformat(),
        "created_by": user["user_id"],
        "created_by_name": user["name"]
    })
    
    updated_loan = await db.loans.find_one({"loan_id": loan_id}, {"_id": 0})
    return {"message": "Loan swapped successfully", "loan": updated_loan}

@api_router.post("/loans/{loan_id}/write-off")
async def write_off_loan(loan_id: str, reason: str = "", user: dict = Depends(require_admin)):
    ...
    if loan["status"] == LoanStatus.FULLY_PAID:
        raise HTTPException(status_code=400, detail="Cannot write off a fully paid loan")
    ...
    await db.loans.update_one(
        ...
        {"$set": {
            ...
            "written_off_date": now.isoformat(),
            "written_off_by": user["name"],
    ...
    await db.loan_transactions.insert_one({
        ...
        "amount": outstanding,
        "currency": loan.get("currency", "USD"),
        "description": f"Loan written off: {reason}",
        "created_at": now.isoformat(),
        "created_by": user["user_id"],
        "created_by_name": user["name"]
    })
    
    return {"message": "Loan written off successfully", "written_off_amount": outstanding}
